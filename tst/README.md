...
This is a tool to quickly generate a sequence of bytes, allowing you to mix float, hex strings, 32-bit integers, 16-bit integers, 8-bit integers and characters in one command.
...
Any number after "\\" will also be parsed as the actual number using 32-bit format. For example

        i = \10         # parsed as i=0x0000000A
        i = \-1         # parsed as i=0xFFFFFFFF

Note: in the comments, the hex number is the actual sequence of bytes that this tool will output, not the ascii digits representing the hex value.
...
Finally, you can use "\\x" to give a sequence of hex values you want to compile. For example

        \x123456        # parsed as 0x12 followed by 0x34
...
Run the same comment with -e flag and see how the endianness changes

        ./mkpckt -e "\f3.14 \b234 \w65123" | xxd -i
        # result: 0xc3, 0xf5, 0x48, 0x40, 0xea, 0x63, 0xfe

Have a look at ./tst/test.txt and inspect the output of the following command
